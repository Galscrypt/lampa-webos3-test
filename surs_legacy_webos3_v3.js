...
(function () {
  ...
  // Chromium 38 compatibility for the externally loaded surs_select.js.
  if (!String.prototype.startsWith) {
    String.prototype.startsWith = function (searchString, position) {
      var subject = String(this);
      var search = String(searchString);
      var start = position > 0 ? Math.floor(position) : 0;
      return subject.slice(start, start + search.length) === search;
    };
  }
  ...
  var Episode = function (data) {
    ...
    this.visible = function () {
      if (card.poster_path) this.img_poster.src = Lampa.Api.img(card.poster_path);else
      if (card.profile_path) this.img_poster.src = Lampa.Api.img(card.profile_path);else
      if (card.poster) this.img_poster.src = card.poster;else
      if (card.img) this.img_poster.src = card.img;else
      this.img_poster.src = './img/img_broken.svg';
      if (card.still_path) this.img_episode.src = Lampa.Api.img(episode.still_path, 'w300');else
      if (card.backdrop_path) this.img_episode.src = Lampa.Api.img(card.backdrop_path, 'w300');else
      if (episode.img) this.img_episode.src = episode.img;else
      if (card.img) this.img_episode.src = card.img;else
  ...
  function startPlugin() {
    ...
    var SourceTMDBrus = function (parent) {
      ...
      this.main = function () {
        ...
        partsData = partsData.slice(0, 36); // Memory-safe SURS RUS cap for legacy webOS.
    ...
    function addSettingMenu() {
      ...
      try {var
        ...
        showButtonsSelectionMenu = function (previousController) {
          try {
            var items = [
        ...
        addMenuButton = function (title, action, icon, callback) {
          try {
        ...
        showSelectionMenu = function (title, items, storagePrefix, keyField, previousController) {
          try {
        ...
        showStreamingSelectionMenu = function (previousController) {
          try {
            var items = [
        ...
        showMoviesByGenreSelectionMenu = function (previousController) {
          try {
            var isUkrainianLanguage = Lampa.Storage.get('language') === 'uk';
            var items = [
        ...
        showTVShowsByGenreSelectionMenu = function (previousController) {
          try {
            var isUkrainianLanguage = Lampa.Storage.get('language') === 'uk';
            var items = [
        ...
        showBestContentByGenreSelectionMenu = function (previousController) {
          try {
            var items = [
        ...
        showBestContentByPeriodSelectionMenu = function (previousController) {
          try {
            var items = [
        ...
        showCirillicMenu = function (previousController) {
          try {
            var key = 'cirillic';
        ...
        showMinVotesMenu = function (previousController) {
          try {
            var key = 'minVotes';
        ...
        showAgeRestrictionsMenu = function (previousController) {
          try {
        ...
        showKeywordFilterMenu = function (previousController) {
          try {
        ...
        };var currentSource = Lampa.Storage.get('source');var sourceName = Lampa.Storage.get('surs_name') || Lampa.Lang.translate('surs_source_name');var sourceNameKids = sourceName + ' ' + Lampa.Lang.translate('surs_source_name_kids').split(' ')[1];var sourceNameRus = sourceName + ' ' + Lampa.Lang.translate('surs_source_name_rus').split(' ')[1];var sourceNameNew = sourceName + ' ' + Lampa.Lang.translate('surs_source_name_new').split(' ')[1]; // Создание объекта для значений выбора источника
        var sourceValues = {};sourceValues[sourceName] = sourceName;sourceValues[sourceNameNew] = sourceNameNew;sourceValues[sourceNameKids] = sourceNameKids;sourceValues[sourceNameRus] = sourceNameRus;Lampa.SettingsApi.addComponent({ component: 'surs', name: Lampa.Lang.translate('surs_collections') + ' ' + sourceName, icon: '<svg height="200px" width="200px" version="1.1" id="_x32_" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 512 512" xml:space="preserve" fill="#000000"><g id="SVGRepo_bgCarrier" stroke-width="0"></g><g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g><g id="SVGRepo_iconCarrier"> <style type="text/css"> .st0{fill:#ffffff;} </style> <g> <path class="st0" d="M443.724,166.599c27.038-2.293,47.087-26.07,44.786-53.125c-2.292-27.038-26.078-47.087-53.115-44.795 c-27.038,2.301-47.078,26.088-44.776,53.124C392.91,148.85,416.677,168.9,443.724,166.599z"></path> <path class="st0" d="M431.752,346.544l30.541-114.485c5.068-19.305-6.466-39.075-25.78-44.144 c-19.304-5.077-39.075,6.448-44.152,25.771v-0.018L365.052,315.64l-78.755-13.276c-17.218-4.304-34.696,5.786-39.578,22.864 l-33.317,133.445c-3.82,13.342,3.913,27.28,17.274,31.1c13.37,3.81,27.298-3.923,31.128-17.283l39.392-98.638l61.286,16.155 C398.863,400.125,421.633,382.927,431.752,346.544z"></path> <path class="st0" d="M388.177,462.949l-0.121-0.01c-0.018,0-0.028,0-0.047,0L388.177,462.949z"></path> <path class="st0" d="M498.349,286.311c-10.1-2.999-20.721,2.749-23.722,12.858l-27.876,93.848 c-2.096,6.606-4.536,11.777-7.146,15.746c-3.987,5.944-8.002,9.373-13.854,12.093c-5.842,2.664-14.031,4.379-25.416,4.37 c-3.009,0.008-6.215-0.113-9.634-0.355l-54.009-3.363c-10.519-0.661-19.575,7.341-20.227,17.861 c-0.662,10.518,7.342,19.574,17.86,20.226l53.73,3.345c4.211,0.298,8.31,0.448,12.28,0.456c10.072-0.009,19.5-0.988,28.369-3.289 c13.268-3.392,25.315-10.127,34.501-19.892c9.251-9.736,15.531-21.885,19.91-35.609l0.074-0.214l28.015-94.362 C514.206,299.923,508.447,289.302,498.349,286.311z"></path> <path class="st0" d="M248.974,81.219L0,21.256v15.14v281.228l248.974-59.962V81.219z M225.123,238.87L23.851,287.355V51.536 l201.272,48.466V238.87z"></path> <polygon class="st0" points="204.989,115.189 47.991,84.937 47.991,253.953 204.989,223.692 "></polygon> </g> </g></svg>' });Lampa.SettingsApi.addParam({ component: 'surs', param: { name: '', type: 'title' }, field: { name: Lampa.Lang.translate('surs_collections') + ' ' + Lampa.Lang.translate('surs_from') + ' ' + sourceName, description: Lampa.Lang.translate('surs_main_update') } });Lampa.SettingsApi.addParam({ component: 'surs', param: { name: 'surs_empty1', type: 'title' }, field: { name: Lampa.Lang.translate('surs_settings_interface'), description: '' } });Lampa.SettingsApi.addParam({ component: 'surs', param: { name: 'surs_setSource', type: 'select', values: sourceValues, default: sourceName }, field: { name: Lampa.Lang.translate('surs_set_as_source'), description: Lampa.Lang.translate('surs_source_description') }, onChange: function (value) {try {console.log('[DEBUG SURS] Выбранный источник:', value);Lampa.Storage.set('source', value);} catch (e) {console.error('Error in setSource onChange:', e);}} });Lampa.SettingsApi.addParam({ component: 'surs', param: { name: 'surs_setButtons', type: 'button' }, field: { name: Lampa.Lang.translate('surs_add_to_menu'), description: Lampa.Lang.translate('surs_menu_description') }, onChange: function () {try {var currentController = Lampa.Controller.enabled().name;showButtonsSelectionMenu(currentController);} catch (e) {console.error('Error in setButtons onChange:', e);}} });var icon = '<svg xmlns="http://www.w3.org/2000/svg" width="2.2em" height="2.2em" viewBox="0 0 48 48">' + '<circle cx="24" cy="24" r="20" fill="white"/>' + '</svg>';setTimeout(addMenuButtons, 100);Lampa.Listener.follow('profile', function (event) {try {if (event.type !== 'changed') {return;}addMenuButtons();} catch (e) {console.error('Error in profile listener:', e);}});Lampa.SettingsApi.addParam({ component: 'surs', param: { name: 'surs_custom_buttons', type: 'button' }, field: { name: Lampa.Lang.translate('surs_custom_buttons'), description: Lampa.Lang.translate('surs_custom_buttons_description') }, onChange: function () {try {var currentController = Lampa.Controller.enabled().name;showSelectionMenu('surs_custom_buttons', getAllButtons(), 'custom_button_', 'id', currentController);} catch (e) {console.error('Error in custom_buttons onChange:', e);}} });Lampa.SettingsApi.addParam({ component: 'surs', param: { name: '', type: 'title' }, field: { name: Lampa.Lang.translate('surs_filters'), description: '' } });Lampa.SettingsApi.addParam({ component: 'surs', param: { name: 'surs_sort_options', type: 'button' }, field: { name: Lampa.Lang.translate('surs_sort_types'), description: Lampa.Lang.translate('surs_sort_description') }, onChange: function () {try {var currentController = Lampa.Controller.enabled().name;showSelectionMenu('surs_sort_types', allSortOptions, 'sort_', 'id', currentController);} catch (e) {console.error('Error in sort_options onChange:', e);}} });Lampa.SettingsApi.addParam({ component: 'surs', param: { name: 'surs_genres', type: 'button' }, field: { name: Lampa.Lang.translate('surs_genres'), description: Lampa.Lang.translate('surs_genres_description') }, onChange: function () {try {var currentController = Lampa.Controller.enabled().name;showSelectionMenu('surs_genres', allGenres, 'genre_', 'id', currentController);} catch (e) {console.error('Error in genres onChange:', e);}} });Lampa.SettingsApi.addParam({ component: 'surs', param: { name: 'surs_global_streaming', type: 'button' }, field: { name: Lampa.Lang.translate('surs_global_streaming'), description: Lampa.Lang.translate('surs_global_streaming_description') }, onChange: function () {try {var currentController = Lampa.Controller.enabled().name;showSelectionMenu('surs_global_streaming', allStreamingServices, 'streaming_', 'id', currentController);} catch (e) {console.error('Error in global_streaming onChange:', e);}} });Lampa.SettingsApi.addParam({ component: 'surs', param: { name: 'surs_rus_streaming', type: 'button' }, field: { name: Lampa.Lang.translate('surs_rus_streaming'), description: Lampa.Lang.translate('surs_rus_streaming_description') }, onChange: function () {try {var currentController = Lampa.Controller.enabled().name;showSelectionMenu('surs_rus_streaming', allStreamingServicesRUS, 'streaming_rus_', 'id', currentController);} catch (e) {console.error('Error in rus_streaming onChange:', e);}} });Lampa.SettingsApi.addParam({ component: 'surs', param: { name: '', type: 'title' }, field: { name: Lampa.Lang.translate('surs_geo_filters'), description: '' } });Lampa.SettingsApi.addParam({ component: 'surs', param: { name: 'surs_streaming', type: 'button' }, field: { name: Lampa.Lang.translate('surs_streaming'), description: Lampa.Lang.translate('surs_region_description') }, onChange: function () {try {var currentController = Lampa.Controller.enabled().name;showStreamingSelectionMenu(currentController);} catch (e) {console.error('Error in streaming onChange:', e);}} });Lampa.SettingsApi.addParam({ component: 'surs', param: { name: 'surs_getMoviesByGenre', type: 'button' }, field: { name: Lampa.Lang.translate('surs_movies'), description: Lampa.Lang.translate('surs_region_description') }, onChange: function () {try {var currentController = Lampa.Controller.enabled().name;showMoviesByGenreSelectionMenu(currentController);} catch (e) {console.error('Error in getMoviesByGenre onChange:', e);}} });Lampa.SettingsApi.addParam({ component: 'surs', param: { name: 'surs_getTVShowsByGenre', type: 'button' }, field: { name: Lampa.Lang.translate('surs_series'), description: Lampa.Lang.translate('surs_region_description') }, onChange: function () {try {var currentController = Lampa.Controller.enabled().name;showTVShowsByGenreSelectionMenu(currentController);} catch (e) {console.error('Error in getTVShowsByGenre onChange:', e);}} });Lampa.SettingsApi.addParam({ component: 'surs', param: { name: 'surs_getBestContentByGenre', type: 'button' }, field: { name: Lampa.Lang.translate('surs_top_all_time'), description: Lampa.Lang.translate('surs_top_content_description') }, onChange: function () {try {var currentController = Lampa.Controller.enabled().name;showBestContentByGenreSelectionMenu(currentController);} catch (e) {console.error('Error in getBestContentByGenre onChange:', e);}} });Lampa.SettingsApi.addParam({ component: 'surs', param: { name: 'surs_best_content', type: 'button' }, field: { name: Lampa.Lang.translate('surs_top_5_years'), description: Lampa.Lang.translate('surs_top_content_description') }, onChange: function () {try {var currentController = Lampa.Controller.enabled().name;showBestContentByPeriodSelectionMenu(currentController);} catch (e) {console.error('Error in best_content onChange:', e);}} });Lampa.SettingsApi.addParam({ component: 'surs', param: { name: '', type: 'title' }, field: { name: Lampa.Lang.translate('surs_technical_settings'), description: '' } });Lampa.SettingsApi.addParam({ component: 'surs', param: { name: 'surs_cirillic', type: 'button' }, field: { name: Lampa.Lang.translate('surs_cyrillic'), description: Lampa.Lang.translate('surs_cyrillic_description') }, onChange: function () {try {var previousController = Lampa.Controller.enabled().name;showCirillicMenu(previousController);} catch (e) {console.error('Error in cirillic onChange:', e);}} });Lampa.SettingsApi.addParam({ component: 'surs', param: { name: 'surs_minVotes', type: 'button' }, field: { name: Lampa.Lang.translate('surs_rating_validation'), description: Lampa.Lang.translate('surs_rating_description') }, onChange: function () {try {var previousController = Lampa.Controller.enabled().name;showMinVotesMenu(previousController);} catch (e) {console.error('Error in minVotes onChange:', e);}} });Lampa.SettingsApi.addParam({ component: 'surs', param: { name: 'surs_ageRestrictions', type: 'button' }, field: { name: Lampa.Lang.translate('surs_age_restriction'), description: Lampa.Lang.translate('surs_age_description') }, onChange: function () {try {var previousController = Lampa.Controller.enabled().name;showAgeRestrictionsMenu(previousController);} catch (e) {console.error('Error in ageRestrictions onChange:', e);}} });Lampa.SettingsApi.addParam({ component: 'surs', param: { name: 'surs_withoutKeywords', type: 'button' }, field: { name: Lampa.Lang.translate('surs_exclude_asian'), description: Lampa.Lang.translate('surs_exclude_asian_description') }, onChange: function () {try {var previousController = Lampa.Controller.enabled().name;showKeywordFilterMenu(previousController);} catch (e) {console.error('Error in withoutKeywords onChange:', e);}} });
            ...
            onChange: function () {
              ...
              Lampa.Input.edit({
                ...
                if (newName && newName.length > 0) {
                  ...
                  setTimeout(function () {return Lampa.Controller.toggle('settings');}, 300);

                  setTimeout(function () {
                    try {softRefresh(newName, false);} catch (e) {}
                  }, 2000);

                  setTimeout(function () {return location.reload();}, 3500);
    ...
    function addMainButton() {
      return; // Keep the native Lampa Home button on the legacy build.
      // Проверка наличия Lampa API
